...
uint8 LPLD_SPI_Init(SPI_InitTypeDef spi_init_structure)
{
  ...
  if(spix == SPI0)
  {
    ...
    if(miso_pin == PTA17)
    {
      PORTA->PCR[17] = 0 | PORT_PCR_MUX(2)| PORT_PCR_DSE_MASK; //SIN
    }
    else //miso_pin == PTC7
    {
      PORTC->PCR[7] = 0 | PORT_PCR_MUX(2) | PORT_PCR_DSE_MASK; //SIN
    } 
  ...
  else if(spix == SPI1)
  {
    ...
    if(miso_pin == PTB17)
    {
      PORTB->PCR[17] = 0 | PORT_PCR_MUX(2)| PORT_PCR_DSE_MASK; //SIN
    }
    else //miso_pin == PTE3
    {
      PORTE->PCR[3] = 0 | PORT_PCR_MUX(2) | PORT_PCR_DSE_MASK; //SIN
    } 
  }
  else if(spix == SPI2)
  {
  ...
  if(spi_mode == SPI_MODE_SLAVE)
  {
   ...
   // spix->CTAR_SLAVE[0] = 0x78000000;   //16位帧
    spix->CTAR_SLAVE[0] = 0x78000000;//| SPI_CTAR_CPOL_MASK|SPI_CTAR_CPHA_MASK;
  }
  else
  {
 ...
 *    读取从机8位的数据
 */
uint32 LPLD_SPI_Master_Read(SPI_Type *spix)
{
  uint8 temp;
 ...
 *      |__SPI_PCS_INACTIVE -片选无效,PCS信号变为高电平
 */
void LPLD_SPI_Master_Write(SPI_Type *spix,uint32 data,uint8 pcsx,uint8 pcs_state)
{  
  /*
  spix->PUSHR = (((uint32_t)(((uint32_t)(pcs_state))<<SPI_PUSHR_CONT_SHIFT)) & SPI_PUSHR_CONT_MASK)
               |SPI_PUSHR_CTAS(0)
               ...
               |data; 
  */
  spix->PUSHR= (0x08010000 | (data & 0xFFFF));
  while(!(spix->SR & SPI_SR_TCF_MASK));
  spix->SR |= SPI_SR_TCF_MASK ;            
}
