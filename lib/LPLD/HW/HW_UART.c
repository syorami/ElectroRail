...
void UART_DMA_Put(UART_Type *uartx,int8* buff,uint16 size)
{

  DMA_InitTypeDef dma0_init_struct;
  
  dma0_init_struct.DMA_CHx = DMA_CH2;                   //使用Ch0通道
  dma0_init_struct.DMA_Req = UART3_TRAN_DMAREQ;         //DMA请求源为ADC0
  dma0_init_struct.DMA_MajorLoopCnt = size;               //主循环计数2次,因为要循环ADC0的AB两组通道
  dma0_init_struct.DMA_MinorByteCnt = 1;                //次循环传输字节计数（由于ADC采样为12位，因此传输2字节）
  dma0_init_struct.DMA_SourceAddr = (uint32)buff;       //源地址：ADC0结果寄存器A地址
  dma0_init_struct.DMA_SourceDataSize = DMA_SRC_8BIT;   //源地址传输数据宽度16位
  dma0_init_struct.DMA_SourceAddrOffset = 1;            //源地址偏移为4个字节，因为ADC0->R寄存为32位宽，A组传输完成后移动到B组
  dma0_init_struct.DMA_LastSourceAddrAdj = 0;           //主循环最后调节地址为-8个字节，因为主循环为2次计数，因此地址偏移了8个字节
  dma0_init_struct.DMA_DestAddr = (uint32)&uartx->D;    //目的地址，即定义的结果保存数组头地址
  dma0_init_struct.DMA_DestDataSize = DMA_DST_8BIT;     //目的地址传输数据宽度16位
  dma0_init_struct.DMA_DestAddrOffset = 0;              //目的地址偏移为2个字节，因为Result为16位变量
  dma0_init_struct.DMA_LastDestAddrAdj=0;               //目的地址最后调节地址为-4个字节
  dma0_init_struct.DMA_AutoDisableReq = TRUE;          //禁用自动禁用请求，即不受主循环计数计数限制
  //初始化DMA
...
void UART_Init(void)
{
  ...
  static DMA_InitTypeDef uart_trn_dma_init_struct;
  term_port_structure.UART_Uartx = TERM_PORT;
  term_port_structure.UART_BaudRate = TERMINAL_BAUD;
  term_port_structure.UART_TxPin=PTE4;
  term_port_structure.UART_RxPin=PTE5;
  term_port_structure.UART_TxDMAEnable=TRUE;
  term_port_structure.UART_TxIntEnable=TRUE;

  
  term_port_structure.UART_RxDMAEnable=TRUE;
  ...
  LPLD_UART_Init(term_port_structure);
   ...
   //配置接收DMA
  uart_trn_dma_init_struct.DMA_CHx = DMA_CH4;                     //选择DMA CH0作为通道
  uart_trn_dma_init_struct.DMA_MajorLoopCnt   = 50;                //设置计数器长度为数组长度
  uart_trn_dma_init_struct.DMA_MinorByteCnt   = 1;                //设置每次的长度为byte
  uart_trn_dma_init_struct.DMA_MajorCompleteIntEnable = TRUE;     //设置DMA 计数器清零中断
  uart_trn_dma_init_struct.DMA_Req            = UART3_REV_DMAREQ; //设置DMA 请求为 UART rev
  uart_trn_dma_init_struct.DMA_DestAddr       = (uint32)&RxBuffer; //设置目的地址为 recv_ram_buf
  uart_trn_dma_init_struct.DMA_DestAddrOffset = 1;                //每一次DMA请求，地址加1
  uart_trn_dma_init_struct.DMA_DestDataSize   = DMA_DST_8BIT;     //设置目的长度为byte
  uart_trn_dma_init_struct.DMA_LastDestAddrAdj= 0;                 //目的地址最后调节
  uart_trn_dma_init_struct.DMA_SourceAddr     = (uint32)&UART3->D;//设置UART D为源地址
  uart_trn_dma_init_struct.DMA_SourceDataSize = DMA_SRC_8BIT;     //设置源长度为byte
  uart_trn_dma_init_struct.DMA_AutoDisableReq = FALSE;            //循环接收
  uart_trn_dma_init_struct.DMA_Isr = dma_isr;
  
  LPLD_DMA_Init(uart_trn_dma_init_struct); //初始化DMA 接收
  LPLD_DMA_EnableReq(DMA_CH4);             //使能DMA请求
  //LPLD_DMA_EnableIrq(uart_trn_dma_init_struct);
}
